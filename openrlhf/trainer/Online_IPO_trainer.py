...
from .ppo_utils import AdaptiveKLController, Experience, FixedKLController, OnlineIPONaiveExperienceMaker, NaiveReplayBuffer


class OnlineIPOPPOTrainer(ABC):
    """
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.rejection_batch_size = rejection_batch_size
        ...
        self.experience_maker = OnlineIPONaiveExperienceMaker(
            ...
            rejection_batch_size=self.rejection_batch_size
        )
    ...
    def fit(
        ...
        steps = 1
        self.strategy.print(f"=========initial steps: {steps}")
        ...
        for episode in range(args.num_episodes):
            ...
            pbar = tqdm(
                range(args.truncate_steps),
                desc=f"Episode [{episode + 1}/{args.num_episodes}]",
                disable=not self.strategy.is_rank_0(),
            )
            self.strategy.print(f"=========prompts-dataloader length: {self.prompts_dataloader.__len__()}")
            self.strategy.print(f"=========truncate_steps: {args.truncate_steps}")

            for rand_prompts in self.prompts_dataloader:
                
                if steps <= args.truncate_steps:
