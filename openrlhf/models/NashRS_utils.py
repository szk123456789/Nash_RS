...
def NashRSRejectionSamplingReward(
    sequences,
    attention_masks,
    inputs,
    kl_coef: float,
    ...
    **generate_kwargs
    ...
    """Calculate preference P(y>pi_star)
    Args:
    ...
    for prompt_index in range(prompt_num):
        ...
        rejection_sample_num = 0
        preference_reward = 0

        while rejection_sample_num < rejection_batch_size:
            sequence_reference, attention_reference, _ = initial_model.generate(**input, **generate_kwargs)
            u = torch.rand(1)
            preference = CalculatePreference(
                reward_model=reward_model,
                sequences1=sequence_actor_prompt,
                sequences2=sequence_reference,
                attention_mask1=attention_mask_actor_prompt,
                attention_mask2=attention_reference
            )
            if preference.mean().item() >= - kl_coef * u.log().item():
                rejection_sample_num += 1
                ...
                preference_reward += preference / rejection_batch_size
